...
function M.assert_type(value, expected_type, name)
  ...
  if actual_type ~= expected_type then
    local message = string.format("%s: expected %s, got %s", name or "value", expected_type, actual_type)
    error(message, 2)
  end
end
...
function M.assert_args(args, specs)
  for i, spec in ipairs(specs) do
    ...
    if not (value == nil and optional) then
      ...
      elseif type(expected_type) == "table" then
        ...
        if not valid then
          error(
            string.format("%s: expected one of %s, got %s", name, table.concat(expected_type, ", "), type(value)),
            2
          )
        end
      end
