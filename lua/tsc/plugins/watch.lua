...
function WatchPlugin:_watch_project(project)
  ...
  local function on_change(err, filename, events)
    if err then
      vim.notify(string.format("Watch error for %s: %s", watch_dir, err), vim.log.levels.ERROR)
      return
    end
  ...
  if success then
    ...
    vim.notify(string.format("Failed to watch %s: %s", watch_dir, err), vim.log.levels.ERROR)
  end
end
...
function WatchPlugin:_handle_file_change(watch_dir, filename, events)
  ...
  self:_debounce_change(full_path, function()
    ...
    if self._config.notify_on_change then
      vim.notify(string.format("File changed: %s", filename), vim.log.levels.INFO)
    end

    -- Clear errors if configured
    if self._config.clear_on_change then
      -- Emit event to clear quickfix
      self._events:emit("tsc.clear_quickfix", {})
    end
...
function WatchPlugin:_debounce_change(key, callback)
  ...
  -- Set new timer
  self._debounce_timers[key] = vim.fn.timer_start(self._config.debounce_ms, function()
