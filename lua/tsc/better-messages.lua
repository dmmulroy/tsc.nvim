---@class BetterMessages
---Module for better error message handling
local M = {}
...
---@return string[] A list of all parameter placeholders found in the template.
...
---@return string[] A list of all quoted strings found in the message.
...
local function parse_md(markdown)
  ...
  if not originalContent or not betterContent then
    return {
      original = "",
      better = "",
    }
...
M.translate = function(message)
  ...
  if #params == 0 and parsed.better then
    return "TS" .. error_num .. ": " .. parsed.better
  end
